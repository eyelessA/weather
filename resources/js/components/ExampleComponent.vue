    ...
    <div class="container flex justify-center items-start min-h-screen mt-16">
        <div>
            <button @click="toggleUnits"
                    class="rounded-md bg-slate-800 py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none mr-2 whitespace-nowrap">
                {{ isKelvin ? 'Switch to Celsius' : 'Switch to Kelvin' }}
            </button>
            <h1 class="rounded-md bg-slate-800 py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none mr-2 mt-2 whitespace-nowrap">
                Recent Searches
            </h1>
            <div v-for="(key, value) in recentSearches" class="mt-2">
        ...
        <div class="w-full max-w-sm min-w-[200px]">
            ...
            <div v-if="selectedCity"
                 class="relative rounded-lg bg-gradient-to-tr from-pink-300 to-blue-300 p-0.5 shadow-lg mt-5">
                <div class="bg-white p-7 rounded-md">
                    <div>
                        <div class="">
                            <div>
                                <h2 class="text-2xl">{{ selectedCity.name }}</h2>
    ...
    methods: {
        ...
        getWeather() {
            ...
            axios.post('/api/get-weather', {city: this.city})
                .then(res => {
                    if (res.data.error === 'city not found') {
